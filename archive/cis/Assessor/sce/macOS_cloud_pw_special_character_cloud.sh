...
pwspecialchar=$(
pref=$(/usr/bin/sudo pwpolicy -getaccountpolicies  | /usr/bin/sudo /usr/bin/grep -A1 minimumSymbols | /usr/bin/sudo /usr/bin/tail -1 | /usr/bin/sudo /usr/bin/cut -d '>' -f2 | /usr/bin/sudo /usr/bin/cut -d '<' -f1) && if [[ "$pref" != "" && pref -ge 1 ]]; then echo "true"; else echo "false"; fi
)
